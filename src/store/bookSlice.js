...
export const booksSlice = createSlice({
  ...
  //       const response = await fetchEntity(endpoint, search);
...
//     try {
//       const response = await updateEntity(id, API_ENDPOINTS.BOOKS, bookData);
//       return response;
//     } catch (err) {
//       return rejectWithValue(err.response ? err.response.data : err.message);
//     }
...
export const deleteBookThunk = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      // await deleteBook(id);
      await deleteEntity(id, API_ENDPOINTS.BOOKS)
      return id;
    } catch (err) {
      return rejectWithValue(err.response ? err.response.data : err.message);
    }
